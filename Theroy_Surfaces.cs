...
public class Theory_Surface : MonoBehaviour
{
    ...
    public int rippleSegmentInterval = 3;
    [Range(3, 12)] public int rippleSegmentCount = 6;
    public float windwardRippleRatio = 0.18f;
    public float leewardRippleRatio = 0.35f;
    [Range(0f, 1f)] public float rippleEmbedRatio = 0.6f;
    [Range(0f, 1f)] public float rippleTipFadeStart = 0.75f;
        ...
        float avgScale, float Aw, float waterValue, Vector3 windDir, float windSpeed)
    {
         ...
         )
        {
            ...
            {
                float t = (float)i / rippleSegmentCount;
                ...
                // +1 = 풍상, -1 = 풍하
                float signedDot = Vector3.Dot(dir, -windCrossNorm);

                // 풍상일수록 0, 풍하일수록 1
                float leewardT = (1f - signedDot) * 0.5f;

                float windwardR = mainRadius * dynamicWindwardRatio;
                float leewardR = mainRadius * dynamicLeewardRatio;

                float rippleRadius = Mathf.Lerp(windwardR, leewardR, leewardT);

                // 풍속이 강할수록 리플 자체도 조금 더 강조
                rippleRadius *= Mathf.Lerp(0.8f, 1.15f, windEffect);
                ...
                float centerDistance = mainRadius + rippleRadius - embed;

                Vector3 rippleWorldPos = centerWorld + dir * centerDistance;

                GameObject ripple = new GameObject($"RippleSeg_{generatedIcicleSegments.Count}");
