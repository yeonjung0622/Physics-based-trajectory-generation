...
public class Theory_Surface : MonoBehaviour
{
    ...
    [Range(0f, 1f)] public float rippleTipFadeStart = 5;
        ...
        float avgScale, float Aw, float waterValue, Vector3 windDir, float windSpeed)
    {
        ...
        void AddRippleMetaballsOnSegment(
    GameObject parentIcicle,
    Vector3 centerWorld,
    Vector3 ortho1,
    Vector3 ortho2,
    Vector3 windCrossNorm,
    float mainRadius,
    float lengthFactor,
    float currentBodyAw,
    ...
    float windSpeed
)
        {
            ...
            if (windCrossNorm.sqrMagnitude < 1e-6f) return;

            // 풍속 영향: 풍속이 reference에 가까워질수록 1에 가까워짐
            float windStrength = Mathf.Clamp01(
                (windSpeed * windSpeed) /
                Mathf.Max(0.0001f, rippleReferenceWindSpeed * rippleReferenceWindSpeed)
            );

            // 단면에 실제로 걸리는 바람 성분까지 반영
            float windEffect = Mathf.Clamp01(windStrength * crossWindAmount);

            // 기존 비대칭 계수도 약간 반영
            float awEffect = Mathf.Clamp01(Mathf.Abs(currentBodyAw));
            float asymEffect = Mathf.Clamp01(windEffect * Mathf.Lerp(0.7f, 1.2f, awEffect));

            // 끝부분으로 갈수록 리플 감소
            float tipFade = 1f - Mathf.SmoothStep(rippleTipFadeStart, 1f, lengthFactor);
            if (tipFade <= 0.01f) return;

            // 바람이 약할 때는 풍상/풍하 차이가 작고,
            // 바람이 강할수록 풍상은 작게, 풍하는 크게
            ...
            // 핵심 변경:
            // 풍상 쪽은 작은 리플을 촘촘하게
            // 풍하 쪽은 큰 리플을 적게 배치해서 완만하게
            int windwardCount = Mathf.Max(
                2,
                Mathf.RoundToInt(Mathf.Lerp(rippleSegmentCount, rippleSegmentCount + 4, asymEffect))
            );

            int leewardCount = Mathf.Max(
                2,
                Mathf.RoundToInt(Mathf.Lerp(rippleSegmentCount, rippleSegmentCount * 0.45f, asymEffect))
            );

            // 풍속이 강할수록 각 리플 분포 영역을 더 좁혀서 방향성이 뚜렷하게 보이게 함
            float windwardArcDeg = Mathf.Lerp(190f, 155f, asymEffect);
            float leewardArcDeg = Mathf.Lerp(200f, 150f, asymEffect);

            // windVector가 "바람이 불어가는 방향"이면
            // +windCrossNorm = 풍하
            // -windCrossNorm = 풍상
            ...
            )
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    // 풍속이 강할수록 전체 리플도 조금 강조
                    rippleRadius *= Mathf.Lerp(0.8f, 1.15f, asymEffect);
                    ...
                    float visibleFactor = Mathf.Clamp((1f - rippleEmbedRatio) * protrudeScale, 0.12f, 0.45f);
                    float centerDistance = mainRadius + rippleRadius * visibleFactor;

                    Vector3 rippleWorldPos = centerWorld + dir * centerDistance;
